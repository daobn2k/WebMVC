...
<form action ="" method="POST" class="formsearch">
  <input type="search" name= "search" class="textbox" placeholder="Vui Lòng Điền Sản Phẩm muốn tìm">
  ...
  if( $search_value !== '') {
    ...
    if($con->connect_error){
        echo 'Connection Faild: '.$con->connect_error;
            ...
            $sql="select * from tbl_product where title_product like '%$search_value%'";
            $res=$con->query($sql);
            $i = 0;
            while($row=$res->fetch_assoc()){
              $id = $row['id_category_product'];
                      $sqlcategory = " SELECT * FROM tbl_category_product WHERE id_category_product = $id ";
                      $result = $con->query($sqlcategory);
                      while($rowcategory =  $result->fetch_assoc()){
              ?>
                <tr>
            <td><?php echo $row['id_product'] ?></td>
            <td><?php echo $row['title_product'] ?></td>
            <td><img src="<?php echo BASE_URL ?>/public/uploads/product/<?php echo $row['image_product'] ?>"height="50" width="50"></td>
            <td><?php echo $rowcategory['title_category_product'] ?></td>
            <td><?php echo number_format($row['price_product'],0,',','.').'đ' ?></td>
            <td><?php echo $row['quantity_product'] ?></td>
            <td><?php 
            if($row['product_hot']==0){
              echo 'Không có';
            }else{
              echo 'Có';
            }
            ?></td>
            <td style="
        word-break: break-all;" ><?php echo $row['desc_product'] ?></td>
            <td>
            <a style="color:#404040;" href="<?php echo BASE_URL ?>/product/delete_product/<?php echo $row['id_product'] ?>"><i class="fas fa-trash"></i></a> || 
            <a style="color:#404040;" href="<?php echo BASE_URL ?>/product/edit_product/<?php echo $row['id_product'] ?>"><i class="far fa-edit"></i></a></td>
          </tr> 
              <?php
               }
            }       
